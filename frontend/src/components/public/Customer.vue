...
<template>
  <div>
        <h5><i class="fa fa-user"></i> الملف الشخصي</h5>
       ...
       <div class="row">
         ...
         <div class="form-group row">
                      ...
                      <div class="col-sm-9">
                        <select v-model="data.customerJobsId" class="form-control">
                           <option
                            v-for="job in jobs"
                            :key="job.id"
                            :value="job.id"
                            :selected="job.id == data.customerJobsId"
                          >{{ job.name }}</option>
                        </select>
                      </div>
                    </div>
         ...
         <div class="form-group row">
                      ...
                      <div class="col-sm-9">
                        <select v-model="data.customerIdentityId" class="form-control">
                           <option
                            v-for="Identity in Identitys"
                            :key="Identity.id"
                            :value="Identity.id"
                            :selected="Identity.id == data.customerIdentityId"
                     ...
                     <div class="col-md-6 offset-6">
                      ...
                      <div class="col-sm-9">
                        <select v-model="data.customerTypeId" class="form-control">
                           <option
                            v-for="type in types"
                            :key="type.id"
                            :value="type.id"
                            :selected="type.id == data.customerTypeId"
                          >{{ type.name }}</option>
                        </select>
                      </div>
                    </div>
...
this.getAllCustomerIdentity();
  ...
  methods: {
getCustomerData(){
customerService.getCustomerData().then(data => {
       this.data = data;
});
},
    ...
    handleSubmit(e) {
      ...
      this.$validator.validate().then(valid => {
        if (valid) {
            ...
            customerService.editCustomer(this.data).then(
              userData => {
                setTimeout(() => {
